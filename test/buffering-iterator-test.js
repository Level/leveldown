const test       = require('tap').test
    ...
    , leveldown  = require('../')
    , collectEntries = testCommon.collectBufferingEntries
...
module.exports.args = function () {
  test('test argument-less iterator#nextBuffering() throws', function (t) {
    var iterator = db.iterator()
    t.throws(
        iterator.nextBuffering.bind(iterator)
      , { name: 'Error', message: 'nextBuffering() requires a callback argument' }
      , 'no-arg iterator#nextBuffering() throws'
    )
    iterator.end(t.end.bind(t))
  })

  test('test argument-less iterator#end() after nextBuffering() throws', function (t) {
    var iterator = db.iterator()
    iterator.nextBuffering(function () {
      t.throws(
...
module.exports.sequence = function () {
  ...
  test('test iterator#nextBuffering after iterator#end() throws', function (t) {
    var iterator = db.iterator()
    iterator.end(function (err) {
      t.notOk(err, 'no error')
      t.throws(
          iterator.nextBuffering.bind(iterator, function () {})
        , { name: 'Error', message: 'cannot call nextBuffering() after end()' }
        , 'no-arg iterator#nextBuffering() after iterator#end() throws'
      )
      t.end()
    })
  })

  test('test twice iterator#nextBuffering() throws', function (t) {
    var iterator = db.iterator()
    iterator.nextBuffering(function (err) {
      t.notOk(err, 'no error')
      iterator.end(function (err) {
        t.notOk(err, 'no error')
        t.end()
    ...
    t.throws(
        iterator.nextBuffering.bind(iterator, function () {})
      , { name: 'Error', message: 'cannot call nextBuffering() before previous nextBuffering() has completed' }
      , 'no-arg iterator#nextBuffering() throws'
    )
  })
}
    ...
    db.batch(data, function (err) {
      ...
      var iterator = db.iterator()
        , fn = function (err, array) {
            ...
            for (; idx < 3; idx++) {
              t.equal(array[idx].key.toString(), data[idx].key, 'correct key')
              t.equal(array[idx].value.toString(), data[idx].value, 'correct value')
            }

            t.equal(array[3], null, 'last element is null')

            iterator.end(function() {
              t.end()
            })
          }
      iterator.nextBuffering(fn)
    })
  })
